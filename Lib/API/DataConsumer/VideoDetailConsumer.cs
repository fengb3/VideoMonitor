...
public class VideoDetailConsumer : IDataConsumer<JsonNode>
{
    public void Consume(JsonNode? data)
    {
        ...
        Log.Info($"解析视频统计信息成功 - {videoRecord.BvId} - {videoRecord.Views} - {videoRecord.Likes} - {videoRecord.Dislikes} - {videoRecord.Coins} - {videoRecord.Favorites} - {videoRecord.Shares} - {videoRecord.Danmaku} - {videoRecord.Comments}");
    }
}